...
def read_icons(dir):
    cars = [
        plt.imread(os.path.join(dir, icon_file))
        for icon_file in os.listdir(dir)
        if not "ROBOT" in icon_file
    ]
    robot = plt.imread(os.path.join(dir, "Car TOP_VIEW ROBOT.png"))
    return cars, robot


try:
    cars, robot = read_icons("../../datasets/raw/v1.0-mini/icons")
except:
    cars, robot = read_icons("../../../datasets/raw/v1.0-mini/icons")
