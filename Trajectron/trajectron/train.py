...
def main():
    ...
    if not os.path.exists(args_conf):
        print('Config json not found!')
    with open(args_conf, 'r', encoding='utf-8') as conf_json:
    ...
    hyperparams['incl_robot_node'] = args_incl_robot_node
    ...
    hyperparams['use_map_encoding'] = args_map_encoding
    hyperparams['augment'] = args_augment
    ...
    if not args.debug:
        ...
        model_dir = os.path.join(args.log_dir,
                                 'models_' + time.strftime('%d_%b_%Y_%H_%M_%S', time.localtime()) + args_log_tag)
    ...
    train_dataset = EnvironmentDataset(train_env,
                                       ...
                                       return_robot=args_return_robot)
    ...
    if args.eval_every is not None:
        eval_data_path = os.path.join(args_data_dir, args_eval_data_dict)
    ...
    for epoch in range(1, args.train_epochs + 1):
        ...
        for node_type, data_loader in train_data_loader.items():
            ...
            for batch in pbar:
                ...
                # Training loss for plot
                loss= train_loss.cpu().detach().numpy().item()
                loss_per_batch.append(loss)

            curr_iter_node_type[node_type] = curr_iter

        epoch_loss = np.array(loss_per_batch).mean().item()
    ...
    plt.title('Training')
    plt.plot(loss_per_epoch, label='Train Loss: ELBO')
    plt.legend(loc='best')
    plt.grid()
    plt.xlabel(f'Number of epochs');
    plt.savefig(model_dir + '/' + f'train_loss_curve_per_epoch')


if __name__ == '__main__':

    if args.dataset == 'openDD':
        import sys
        sys.path.append('../experiments/nuScenes')
        args_data_dir = '../experiments/processed/openDD_processed_vehicle'
        args_train_data_dict = 'openDD_train_full.pkl'
        args_eval_data_dict = 'openDD_val_full.pkl'

    elif 'nuScenes' in args.dataset:
        args_data_dir = '../experiments/processed/nuScenes_US_Processed'
        if "mini" in args.dataset:
            args_train_data_dict = 'nuScenes_train_mini_full.pkl'
            args_eval_data_dict = 'nuScenes_val_mini_full.pkl'
        else:
            args_train_data_dict = 'nuScenes_train_full.pkl'
            args_eval_data_dict = 'nuScenes_val_full.pkl'


    # if args.model_type == 'velocity':
    #     args_log_tag = '_vel_ee'
    #     args_conf = '../experiments/nuScenes/models/vel_ee/config.json'
    #     args_augment = True
    #     args_map_encoding = False
    #     args_incl_robot_node = False
    #
    # elif model_type == 'dynamics integration':
    #     args_log_tag = '_int_ee'
    #     args_conf = '../experiments/nuScenes/models/int_ee/config.json'
    #     args_augment = True
    #     args_map_encoding = False
    #     args_incl_robot_node = False
    #
    # elif model_type == 'dynamics integration robot':
    #     args_log_tag = '_int_ee_rob'
    #     args_conf = '../experiments/nuScenes/models/int_ee/config.json'
    #     args_augment = False
    #     args_map_encoding = False
    #     args_incl_robot_node = True

    if args.model_type == 'int_ee_me':
        args_conf = '../experiments/nuScenes/models/int_ee_me/config.json'
        args_augment = True
        args_map_encoding = True
        args_incl_robot_node = False

    elif args.model_type == 'robot':
        args_conf = '../experiments/nuScenes/models/robot/config.json'
        args_augment = False
        args_map_encoding = True
        args_incl_robot_node = True

    args_log_tag = f'_{args.model_type}_{args.dataset}'

    main()
#python train.py --model_type int_ee_me --dataset openDD  --eval_every 1 --vis_every 1  --ph 12 --max_hl 4 --lr 0.002 --offline_scene_graph yes --preprocess_workers 0 --batch_size 256 --log_dir ../experiments/nuScenes/models --train_epochs 20 --node_freq_mult_train  --device gpu
