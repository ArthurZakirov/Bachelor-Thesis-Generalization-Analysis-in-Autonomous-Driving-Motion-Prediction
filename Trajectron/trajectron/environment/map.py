...
class GeometricMap(Map):
    ...
    def get_cropped_maps_from_scene_map_batch(cls, maps, scene_pts, patch_size, rotation=None, device='cpu'):
        ...
        layer_indices = [cls.layer_dict[layer_name] for layer_name in maps[0].use_layers]
        padded_map_batched = padded_map_batched[:, layer_indices]
        ...
        try:
            ...
            print('Something wrong with the map!')
            print('padded_map_batched.shape:', padded_map_batched.shape)
            print('padded_map_batched.sum:  ', padded_map_batched.sum())
            print('center_patches.shape:    ', center_patches.shape)
            print('center_patches.sum:      ', center_patches.sum())
            print(exc)
