...
def trajectron_const_a(
    ...
    for node_type in self.env.NodeType:
        ...
        # Run forward pass
        def const_acceleration_predict(x_t, ph, num_samples):
            x_current = x_t[:, -1, :2]
            v_current = x_t[:, -1, 2:4]
            a_current = x_t[:, -1, 4:6]
            dt = 0.5
            pred = torch.stack(
                [
                    x_current
                    + v_current * (t_step * dt)
                    + 1 / 2 * a_current * (t_step * dt) ** 2
                    for t_step in range(ph)
                ],
                dim=1,
            )
            pred = pred.repeat(num_samples, 1, 1, 1)
            return pred

        predictions_np = const_acceleration_predict(x, ph, num_samples).cpu().numpy()
        # Assign predictions to node
        for i, ts in enumerate(timesteps_o):
            if ts not in predictions_dict.keys():
                predictions_dict[ts] = {}
