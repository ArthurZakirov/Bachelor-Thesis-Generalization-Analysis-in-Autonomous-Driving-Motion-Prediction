...
class DiscreteLatent(object):
    ...
    def sample_p(self, num_samples, mode, most_likely_z=False, full_dist=True, all_z_sep=False, min_k=False):
        num_components = 1
        if full_dist and not min_k:
            bs = self.p_dist.probs.size()[0]
            z_NK = torch.from_numpy(self.all_one_hot_combinations(self.N, self.K)).float().to(self.device).repeat(num_samples, bs)
            ...
            k = num_samples
            eye_mat = torch.eye(self.p_dist.event_shape[-1], device=self.device) # [25,25]
            _,topk_idxs = self.p_dist.probs.topk(k, dim=2) #
            z_NK = torch.unsqueeze(eye_mat[topk_idxs], dim=0)
